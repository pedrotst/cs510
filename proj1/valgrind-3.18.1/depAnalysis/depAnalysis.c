...
void printf_int_list(int_node *current){
  while (current != NULL) {
...
void printf_read_dep_list(read_dep_node* head) {
  ...
  while (current != NULL) {
    if (current->read_deps != NULL)
    {
      ...
      else if (current->val.tag == k_write)
      {
        ...
        printf_int_list(current->read_deps);
        VG_(fprintf)(fp, "\n");
      }
...
void read_dep_node_erase_tmp(Int tmp)
{
  if(var_deps == NULL) return;
...
static void instrument_assgn_tmp_unop(UInt tmp_lhs, UInt tmp_rhs)
{
  ...
  read_dep_node_erase_tmp(tmp_lhs);
                           ...
                           IRType gWordTy, IRType hWordTy)
{
  ...
  for (i = 0; i < sbIn->stmts_used; i++)
  {
    ...
    case Ist_Put:
      ...
      if(trace){
        argv = mkIRExprVec_0();
        dirty = unsafeIRDirty_0_N(0, "ta_exit", VG_(fnptr_to_fnentry)(instrument_exit), argv);
        addStmtToIRSB(sbOut, IRStmt_Dirty(dirty));
      }
      break;
    default: break;
...
static void da_fini(Int exitcode)
{
  ...
  VG_(printf)("--------- Printing registers --------- \n");
  print_read_dep_list(reg_deps);
  VG_(printf)("--------- Printing temps --------- \n");
  print_read_dep_list(var_deps);
  VG_(printf)("--------- Printing writes --------- \n");
  print_read_dep_list(write_deps);
  printf_read_dep_list(write_deps);
...
static void ta_pre_call(ThreadId id, UInt syscallno, UWord *args, UInt nargs)
{
  if (trace)
  {
    ...
    if (syscallno == __NR_write)
    {
      ...
      VG_(printf)("Creating write (%d) dep\n", write_n);
